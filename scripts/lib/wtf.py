...
def split_csv(file, functionName, ret=False):
    ...
    with open(file, 'rb') as csvfilein:
        ...
        count = 0
        firstRound = True
        splitChunks = []  # the csv file divided by functionName
        ...
        for row in reader:
            ...
            if function == functionName:
                ...
                if not ret:
                    ...
                    splitChunks.append(csvChunks[:-1])
                csvChunks = csvChunks[-1:]
                count += 1

        if ret:
            print 'Returning {} chunks'.format(len(splitChunks))
            return splitChunks
